...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  /**
   * Recursive method that eliminates red property violations associated with the newlyAdded node as
   * well as eliminating red property violations of the newlyAdded's grandparent node and its
   * grandparent and so on.
   * 
   * 
   * @param newlyAdded - Node<T> whose red property violations will be fixed by the method
   */
  private void enforceRBTreePropertiesAfterInsert(Node<T> newlyAdded) {
    // red property violations only happen if the parent is red

    if (newlyAdded != null && newlyAdded.parent != null && !newlyAdded.parent.isBlack) {
      // getting the parent's sibling
      Node<T> parentsSibling = null;
      if (newlyAdded.parent.isLeftChild())
        parentsSibling = newlyAdded.parent.parent.rightChild;
      else
        parentsSibling = newlyAdded.parent.parent.leftChild;

      // if the parents sibling is black (null included)
      if (parentsSibling == null || parentsSibling.isBlack) {
        // if the relations between GP, P, C are left - right or right-left, must do a child
        // parent rotation

        Node<T> subtreeRoot = newlyAdded.parent;
        if ((newlyAdded.parent.isLeftChild() && !newlyAdded.isLeftChild())
            || (!newlyAdded.parent.isLeftChild() && newlyAdded.isLeftChild())) {
          rotate(newlyAdded, newlyAdded.parent);
          rotate(newlyAdded, newlyAdded.parent);
          subtreeRoot = newlyAdded;
        } else
          rotate(newlyAdded.parent, newlyAdded.parent.parent);

        subtreeRoot.isBlack = true;
        if (subtreeRoot.leftChild != null)
          subtreeRoot.leftChild.isBlack = false;
        if (subtreeRoot.rightChild != null)
          subtreeRoot.rightChild.isBlack = false;


      } else if (!parentsSibling.isBlack) {
        if (newlyAdded.parent.parent != null) {
          newlyAdded.parent.parent.isBlack = false;
          newlyAdded.parent.isBlack = true;
          parentsSibling.isBlack = true;

        }

      }

      if (newlyAdded.parent != null)
        enforceRBTreePropertiesAfterInsert(newlyAdded.parent.parent);
  ...
  private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    if (!child.parent.equals(parent))
      throw new IllegalArgumentException("Child is not child of given parent");

    if (child.isLeftChild()) {

      Node<T> saveChildsRight = child.rightChild;
      child.rightChild = parent;
      parent.leftChild = saveChildsRight;
      ...
      if (saveChildsRight != null)
        saveChildsRight.parent = parent;
    } else {
      Node<T> saveChildsLeft = child.leftChild;
      child.leftChild = parent;
      parent.rightChild = saveChildsLeft;
      ...
      if (saveChildsLeft != null)
        saveChildsLeft.parent = parent;
    }
